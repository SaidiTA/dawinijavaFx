...
public List<Patient> recupererbymedecin(int id) throws SQLException{
         String req = "SELECT * FROM user u JOIN consulation c ON u.id = c.patients_id WHERE JSON_SEARCH(u.roles, 'one', 'ROLE_PATIENT') IS NOT NULL AND c.medecin_id = ? GROUP BY c.patients_id;";
    PreparedStatement pst = cnx.prepareStatement(req);
        pst.setInt(1, id);
    ResultSet rs = pst.executeQuery();
