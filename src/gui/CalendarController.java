...
ZonedDateTime dateFocus;
    ...
    private void drawCalendar() throws SQLException{
        ...
        double spacingV = calendar.getVgap();
          ConsulationService as = new ConsulationService();
        //List of activities for a given month
        Map<Integer, List<Consulation>> calendarActivityMap = createCalendarMap(as.getCalendarActivitiesMonth(dateFocus));
            ...
            for (int j = 0; j < 7; j++) { 
                ...
                if(calculatedDate > dateOffset){
                    ...
                    if(currentDate <= monthMaxDate){
                      ...
                      List<Consulation> calendarActivities = calendarActivityMap.get(currentDate);
    ...
    private void createCalendarActivity(List<Consulation> Activites, double rectangleHeight, double rectangleWidth, StackPane stackPane){
    VBox vBox = new VBox();
    ...
    Text dateText = new Text(String.valueOf(Activites.get(0).getHeuredebut().getDate()));
    ...
    private Map<Integer, List<Consulation>> createCalendarMap(List<Consulation> calendarActivities) {
        Map<Integer, List<Consulation>> calendarActivityMap = new HashMap<>();

        for (Consulation activity: calendarActivities) {
            int activityDate = activity.getHeuredebut().getDate();
            if(!calendarActivityMap.containsKey(activityDate)){
                calendarActivityMap.put(activityDate, Arrays.asList(activity));
            } else {
                List<Consulation> OldListByDate = calendarActivityMap.get(activityDate);

                List<Consulation> newList = new ArrayList<>(OldListByDate);
