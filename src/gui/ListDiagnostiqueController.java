...
public class ListDiagnostiqueController implements Initializable {
    ...
    // Ajout de cette variable pour stocker l'ID du dossier sélectionné
    ...
    private void Recherche(KeyEvent event) {
         DiagnostiqueCrud se = new DiagnostiqueCrud();
        String chaine = Recherche.getText();
        populateTable(se.chercherDiagnostique(chaine));
    }
    
      private void populateTable(ObservableList<Diagnostique> chercherDiagnostique) {
     dos.getChildren().clear();
     dos.getChildren().add(HB);
     if (chercherDiagnostique.isEmpty()) {
         // Créer le label avec le message "Aucun diag trouvé"
         Label aucunDossierLabel = new Label("Aucun diagnostique trouvé");
         ...
         for(Diagnostique diagnostique: chercherDiagnostique){
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("Diagnostique_item.fxml"));
            try{
                HBox hBox =fxmlLoader.load();
                Diagnostique_itemController dic= fxmlLoader.getController();
