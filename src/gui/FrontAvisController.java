...
private void submitComment(ActionEvent event) throws SQLException {
   ...
   Avis avis = new Avis();
    ...
    avis.setPatient_id(UserSession.getInstance().getCurrentUser().getId());
...
avis.setPatient(patient);
   ...
   private void refreshAvis() throws SQLException {
          allAVIS.getChildren().clear();
    AvisService avisService = new AvisService();
    List<Avis> avisList = avisService.recupererAvisMedecin(medecin.getId());
    setAvis(avisList);
