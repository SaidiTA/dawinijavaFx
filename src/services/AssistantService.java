...
pst2.executeUpdate();
    ...
    public List<Assistant> getAssistantsForMedecin(int idMedecin) throws SQLException {
    List<Assistant> allAssistants = new AssistantService().recuperer();
    List<Assistant> assistantsForMedecin = new ArrayList<>();
    
    for (Assistant assistant : allAssistants) {
        if (assistant.getMedecin() != null && assistant.getMedecin().getId() == idMedecin) {
            assistantsForMedecin.add(assistant);
        }
    }
    
    return assistantsForMedecin;
}
}
